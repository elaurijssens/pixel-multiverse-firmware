...
namespace display {
    ...
    void selftest(uint8_t test_id) {
        // Renders locally from a tiny command payload, so it works even where a
        // full `data` frame's bulk USB read would be throttled by the refresh.
        // Note: some patterns (01 white, solid fills, gradients) light most LEDs
        // and draw a lot of current — S5.5 will make these size/power aware.
